...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    try {
      ...
      const startDate = '2025-10-01'; // Membership period start
      ...
      for (const member of members) {
        ...
        const speechCount = db.prepare(`
          ...
          WHERE s.speaker_id = ? AND m.date >= ?
        `).get(member.id, startDate).count;
